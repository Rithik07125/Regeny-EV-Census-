...
const SHABIK_IDS = [4, 6, 8, 9, 60, 63, 67, 68, 69, 72, 73, 84, 85, 109, 111, 113, 114, 116, 120, 122, 127, 135, 147, 148, 149, 151, 152, 153, 156];
...
function extractIds(operator, raw) {
  ...
  try {
    switch (operator) {
      ...
      case 'shabik': {
        // Same Ampeco-style API shape as UAEV
        if (Array.isArray(raw)) {
          return raw.map(l => ({ id: String(l.id ?? ''), name: l.name || l.title || '', lat: l.latitude, lng: l.longitude })).filter(x => x.id);
        }
        if (raw && typeof raw === 'object') {
...
// ---- Shabik ----
async function fetchShabik() {
  const locObj = {};
  SHABIK_IDS.forEach(id => locObj[id] = '');
  const body = JSON.stringify({ locations: locObj });
  const result = await httpsPost('shabikuae.eu.charge.ampeco.tech', '/api/v1/app/locations?operatorCountry=AE',
    {
      'Accept': 'application/json, text/plain, */*',
      'Content-Type': 'application/json',
      'x-operator-country': 'AE',
      'x-mobile-app-bundle-id': 'net.shabik.ae.app',
      'x-platform': 'ios',
      'User-Agent': 'ChargeMobile/1781650767 CFNetwork/3860.600.12 Darwin/25.5.0'
    },
    body
  );
  return result ? result.locations : null;
...
async function refreshFastCaches() {
  const [ion, uaev, zynetic, dewa, shabik] = await Promise.all([fetchIon(), fetchUaev(), fetchZynetic(), fetchDewa(), fetchShabik()]);
  if (ion !== null) ionCache = ion;
  if (uaev !== null) uaevCache = uaev;
  if (zynetic !== null) zyneticCache = zynetic;
  if (dewa !== null) dewaCache = dewa;
  if (shabik !== null) shabikCache = shabik;
}
...
async function takeSnapshot() {
  ...
  const [ion, uaev, e2go, zynetic, dewa, shabik] = await Promise.all([
    ...
    shabikCache !== null ? Promise.resolve(shabikCache) : fetchShabik()
  ]);
  const snapshot = {
    date,
    ...
    operators: {
      ...
      shabik: extractIds('shabik', shabik),
    }
  };
  try {
...
http.createServer(async (req, res) => {
  ...
  if (req.url.startsWith('/ion')) {
  ...
  } else if (req.url.startsWith('/shabik')) {
    const data = shabikCache !== null ? shabikCache : await fetchShabik();
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify(data));
    ...
    res.writeHead(404);
    res.end('Use /ion, /uaev, /e2go, /zynetic, /dewa, /shabik, /updates, /snapshots, or /health');
  }
}).listen(PORT, () => {
